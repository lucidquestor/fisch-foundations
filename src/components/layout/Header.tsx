  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-cream-deeper bg-ivory/95 backdrop-blur-md">
      <div className="container-site flex items-center justify-between px-5 py-3 md:px-10 lg:px-16">
        <Link href="/" className="font-display text-xl tracking-wide text-navy">
          FISCH
          <span className="ml-2 text-[10px] font-sans font-semibold uppercase tracking-widest2 text-stone">
            Foundations
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-10 lg:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-xs font-medium uppercase tracking-[0.18em] text-stone transition-colors hover:text-navy"
            >
              {link.label}
            </Link>
          ))}
